...
def create_category_selector_message(selected_index: int, normal_configured: bool = False) -> str:
    ...
    categories = [
        ...
        (
            ...
            "Complex reasoning - falls back to Normal if not set",
        ),
        (
            "critique",
            "Critique Model [Optional]",
            "Self-critique of reasoning - falls back to Thinking",
        ),
        (
            "compact",
            "Compact Model [Optional]",
            "Context compaction - falls back to Normal if not set",
        ),
        (
            "vlm",
            "Vision Model [Optional]",
            "Image processing - vision tasks unavailable if not set",
        ),
    ...
    lines.append(
        f"{CYAN}│{RESET} {BOLD}Choose which model slot to configure:{RESET}{' ' * 32}│{RESET}"
    )
    ...
    lines.append(f"{CYAN}│{' ' * BOX_WIDTH}│{RESET}")
    lines.append(
        f"{CYAN}│{RESET} {DIM}Use ↑/↓ arrows or j/k, Enter to select{RESET}{' ' * 27}│{RESET}"
    )
    ...
    for idx, (category_id, name, description) in enumerate(categories):
        ...
        if is_disabled:
            ...
            if is_selected:
                ...
                desc_padding = BOX_WIDTH - len(desc_line) - 1
                lines.append(
                    f"{CYAN}│{RESET} {REVERSE}{MAGENTA}{name_line}{' ' * name_padding}{RESET}│{RESET}"
                )
                lines.append(
                    f"{CYAN}│{RESET} {REVERSE}{MAGENTA}{desc_line}{' ' * desc_padding}{RESET}│{RESET}"
                )
            else:
                name_line = f"  {name}"
                desc_line = f"  {description}"
                ...
                desc_padding = BOX_WIDTH - len(desc_line) - 1
                lines.append(
                    f"{CYAN}│{RESET} {MAGENTA}{name_line}{RESET}{' ' * name_padding}│{RESET}"
                )
                lines.append(f"{CYAN}│{RESET} {DIM}{desc_line}{RESET}{' ' * desc_padding}│{RESET}")
        elif is_selected:
            ...
            desc_padding = BOX_WIDTH - len(desc_line) - 1
            lines.append(
                f"{CYAN}│{RESET} {REVERSE}{GREEN}{name_line}{' ' * name_padding}{RESET}│{RESET}"
            )
            lines.append(
                f"{CYAN}│{RESET} {REVERSE}{GREEN}{desc_line}{' ' * desc_padding}{RESET}│{RESET}"
            )
        else:
...
def get_category_items(
    ...
    categories = [
        ...
        ("critique", "Critique Model [Optional]", "Self-critique of reasoning"),
        ("compact", "Compact Model [Optional]", "Context compaction summaries"),
