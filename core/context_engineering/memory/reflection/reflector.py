...
class ExecutionReflector:
    ...
    def _extract_testing_pattern(
        ...
        for tc in tool_calls:
            if tc.name == "run_command":
                ...
                if any(
                    keyword in cmd.lower() for keyword in ["test", "pytest", "jest", "npm test"]
                ):
    ...
    def _extract_shell_command_pattern(
        ...
        commands = [
            tc.parameters.get("command", "") for tc in tool_calls if tc.name == "run_command"
        ]
        ...
        has_run = any(any(keyword in cmd.lower() for keyword in run_keywords) for cmd in commands)
