...
def test_preserve_workspace_path():
    ...
    result = _sanitize_local_paths(args)
    assert (
        result["path"] == "/workspace/src/model.py"
    ), f"Expected '/workspace/src/model.py', got '{result['path']}'"
...
def test_sanitize_pdf_path():
    ...
    ), f"Expected '2303.11366v4.pdf', got '{result['path']}'"
...
def test_sanitize_multiple_args():
    ...
    assert (
        result["output_path"] == "output.txt"
    ), f"Expected 'output.txt', got '{result['output_path']}'"
...
def test_translate_relative_path():
    ...
    assert (
        result == "/workspace/src/model.py"
    ), f"Expected '/workspace/src/model.py', got '{result}'"
...
def test_rewrite_removes_local_keyword():
    ...
    assert (
        "local PDF" not in result
    ), f"'local PDF' should be removed, got task body containing 'local PDF'"
