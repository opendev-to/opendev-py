...
def create_model_selector_message(selected_index: int, selection_mode: str = "normal") -> str:
    ...
    items: List[Tuple[str, str, str, str, str]] = (
        []
    )  # (type, provider_id, model_id, display_text, category)
    ...
    items.append(("back", "", "", "← Back to category selection", selection_mode))
    ...
    for provider_info in registry.list_providers():
        ...
        for model_info in matching_models:
            ...
            items.append(("model", provider_info.id, model_info.id, display, selection_mode))
    ...
    lines.append(
        f"{CYAN}│{RESET} {BOLD}Use ↑/↓ arrows or j/k to navigate, Enter to select{RESET}{' ' * 24}│{RESET}"
    )
    ...
    for idx, (item_type, provider_id, model_id, display_text, category) in enumerate(items):
        ...
        if item_type == "back":
            ...
            if is_selected:
                lines.append(
                    f"{CYAN}│{RESET} {REVERSE}{MAGENTA}{display_text}{' ' * padding}{RESET}│{RESET}"
                )
            else:
                lines.append(
                    f"{CYAN}│{RESET} {MAGENTA}{display_text}{RESET}{' ' * padding}│{RESET}"
                )
        elif item_type == "provider":
            # Provider header
            if is_selected:
                lines.append(
                    f"{CYAN}│{RESET} {REVERSE}{YELLOW}{display_text}{' ' * padding}{RESET}│{RESET}"
                )
            else:
            ...
            if is_selected:
                lines.append(
                    f"{CYAN}│{RESET} {REVERSE}{GREEN}{display_text}{' ' * padding}{RESET}│{RESET}"
                )
            else:
...
def get_model_items(selection_mode: str = "normal"):
    ...
    # Map selection mode to required capabilities
    mode_capabilities = {"normal": ["text"], "thinking": ["reasoning"], "vlm": ["vision"]}
    ...
    for provider_info in registry.list_providers():
        ...
        for model_info in matching_models:
            items.append(
                ("model", provider_info.id, model_info.id, model_info.name, selection_mode)
            )
