    ...
    def list_sessions(
        ...
        if index is not None:
            if not include_archived:
                entries = [e for e in index["entries"] if not e.get("timeArchived")]
            else:
            ...
            if not include_archived:
                ...
                if re_index is not None:
                    entries = [e for e in re_index["entries"] if not e.get("timeArchived")]
                    sessions = [self._metadata_from_index_entry(e) for e in entries]
                        ...
                        for entry in data["entries"]:
                            ...
                            if sid and sid not in seen_ids:
                                if not include_archived and entry.get("timeArchived"):
                                    continue
                                seen_ids.add(sid)
                                all_sessions.append(self._metadata_from_index_entry(entry))
                        continue
