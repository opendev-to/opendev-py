...
class BridgeUICallback(BaseUICallback):
    ...
    def on_tool_result(
        ...
        if hasattr(self._tui, "on_tool_result"):
            try:
                self._tui.on_tool_result(
                    tool_name, tool_args, result, tool_call_id=tool_call_id
                )
            except TypeError:
                self._tui.on_tool_result(tool_name, tool_args, result)
        self._forward_web(
            "on_tool_result", tool_name, tool_args, result, tool_call_id=tool_call_id
        )
    ...
    def on_nested_tool_call(
        ...
        self._forward_web(
            "on_nested_tool_call", tool_name, tool_args, depth=depth, parent=parent
        )
    ...
    def on_single_agent_complete(
        ...
        self._tui.on_single_agent_complete(
