...
class SWECLIChatApp(App):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._model_picker: ModelPickerController = ModelPickerController(
            self, self._interrupt_manager
        )
        self._agent_creator: AgentCreatorController = AgentCreatorController(
            self, self._interrupt_manager
        )
        self._skill_creator: SkillCreatorController = SkillCreatorController(
            self, self._interrupt_manager
        )
    ...
    def _show_interrupt_feedback(self) -> None:
        ...
        if not getattr(self, "_interrupt_message_written", False) and hasattr(self, "conversation"):
    ...
    def action_toggle_parallel_expansion(self) -> None:
        ...
        2. For completed items, toggle whichever is most recent (higher line number)
        """
        # First priority: active parallel agent groups (in-progress)
        ...
        # For completed items, compare recency by line number
        agent_line = self.conversation.get_single_agent_line()
        collapsible_line = self.conversation.get_most_recent_collapsible_line()

        if agent_line is not None and collapsible_line is not None:
            if collapsible_line > agent_line:
                self.conversation.toggle_output_expansion()
            else:
                self.conversation.toggle_single_agent_expansion()
        elif agent_line is not None:
            self.conversation.toggle_single_agent_expansion()
        elif collapsible_line is not None:
            self.conversation.toggle_output_expansion()
