...
class TestTodoHandlerFindTodo:
    ...
    def handler(self):
        ...
        handler.create_todo(
            title="Design core game mechanics (jumping, running, collision detection)"
        )
    ...
    def test_find_by_exact_title_case_insensitive(self, handler):
        ...
        actual_id, todo = handler._find_todo(
            "DESIGN CORE GAME MECHANICS (JUMPING, RUNNING, COLLISION DETECTION)"
        )
        assert actual_id == "todo-2"
        assert todo is not None
...
class TestTodoMarkdownStripping:
    ...
    def test_create_todo_strips_active_form(self):
        ...
        assert (
            handler._todos["todo-1"].active_form == "extending PreloadScene: preload tilemap JSON"
        )
