...
def test_modular_composition_loads_all_sections():
    ...
    templates_dir = Path(__file__).parent.parent.parent / "opendev/core/agents/prompts/templates"
...
def test_modular_files_have_frontmatter():
    ...
    templates_dir = (
        Path(__file__).parent.parent.parent / "opendev/core/agents/prompts/templates/system/main"
    )

    modular_files = list(templates_dir.glob("*.md"))
    assert (
        len(modular_files) == 21
