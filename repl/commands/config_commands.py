...
class ConfigCommands(CommandHandler):
    ...
    async def _show_category_selector(self) -> Optional[str]:
        ...
        self.chat_app.model_selector_modal_manager._is_category_selector = (
            True  # Mark as category selector
        )
    ...
    def _switch_to_model(
        self, provider_id: str, model_id: str, mode: str = "normal"
    ) -> CommandResult:
    ...
    def _show_model_config_summary(self) -> None:
        ...
        if config.model_thinking:
            ...
            thinking_provider = (
                config.model_thinking_provider.capitalize()
                if config.model_thinking_provider
                else "Unknown"
            )
        ...
        if config.model_vlm:
            ...
            vlm_provider = (
                config.model_vlm_provider.capitalize() if config.model_vlm_provider else "Unknown"
            )
        ...
        if config.model_critique:
            ...
            critique_provider = (
                config.model_critique_provider.capitalize()
                if config.model_critique_provider
                else "Unknown"
            )
        ...
        if config.model_compact:
            ...
            compact_provider = (
                config.model_compact_provider.capitalize()
                if config.model_compact_provider
                else "Unknown"
            )
