...
class ThinkingMixin:
    ...
    def _get_thinking_trace(
        ...
        try:
            ...
            # Build analysis prompt — phase-aware selection
            todo_handler = getattr(tool_registry, "todo_handler", None) if tool_registry else None
            if plan_approved:
                # Post-plan execution: no tool-delegation instructions
                # (thinking phase has no tools, and the agent already has full context)
                analysis_content = get_reminder(
                    "thinking_analysis_prompt_plan_execution",
                    original_task=original_task,
                )
            elif todo_handler and todo_handler.has_todos():
