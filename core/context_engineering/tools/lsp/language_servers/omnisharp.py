...
from opendev.core.context_engineering.tools.lsp.ls_utils import (
    DotnetVersion,
    FileUtils,
    PlatformId,
    PlatformUtils,
...
class OmniSharp(SolidLanguageServer):
    ...
    def _get_initialize_params(repository_absolute_path: str) -> InitializeParams:
        ...
        """
        with open(
            os.path.join(os.path.dirname(__file__), "omnisharp", "initialize_params.json"),
            encoding="utf-8",
        ) as f:
            d = json.load(f)
    ...
    def _setup_runtime_dependencies(
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "omnisharp", "runtime_dependencies.json"),
            encoding="utf-8",
        ) as f:
            d = json.load(f)
            del d["_description"]
        ...
        if not os.path.exists(razor_omnisharp_ls_dir):
            ...
            FileUtils.download_and_extract_archive(
                runtime_dependencies["RazorOmnisharp"]["url"], razor_omnisharp_ls_dir, "zip"
            )
    ...
    def _start_server(self) -> None:
        ...
        with open(
            os.path.join(
                os.path.dirname(__file__), "omnisharp", "workspace_did_change_configuration.json"
            ),
            encoding="utf-8",
        ) as f:
