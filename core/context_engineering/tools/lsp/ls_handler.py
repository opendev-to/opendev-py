...
from opendev.core.context_engineering.tools.lsp.lsp_protocol_handler.lsp_requests import (
    LspNotification,
)
...
from opendev.core.context_engineering.tools.lsp.util.subprocess_util import (
    quote_arg,
    subprocess_kwargs,
)
    ...
    def _read_ls_process_stdout(self) -> None:
        ...
        try:
            ...
            exception = LanguageServerTerminatedException(
                "Language server process terminated while reading stdout", self.language, cause=e
            )
        ...
        if not self._is_shutting_down:
            if exception is None:
                exception = LanguageServerTerminatedException(
                    "Language server stdout read process terminated unexpectedly", self.language
                )
