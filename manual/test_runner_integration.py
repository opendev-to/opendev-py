...
def test_runner_initialization():
    ...
    try:
        ...
        print(
            f"   Agent initialized: {hasattr(runner.repl, 'agent') and runner.repl.agent is not None}"
        )
        print(
            f"   Query processor initialized: {hasattr(runner.repl, 'query_processor') and runner.repl.query_processor is not None}"
        )
        print(
            f"   Tool registry initialized: {hasattr(runner.repl, 'tool_registry') and runner.repl.tool_registry is not None}"
        )
        ...
        if hasattr(runner.repl, "query_processor") and runner.repl.query_processor:
            ...
            print(
                f"   Has process_query method: {hasattr(runner.repl.query_processor, 'process_query')}"
            )
        ...
        try:
            ...
            if new_messages:
                ...
                for i, msg in enumerate(new_messages, 1):
                    print(
                        f"   {i}. [{msg.role.value}] {msg.content[:100]}..."
                        if len(msg.content) > 100
                        else f"   {i}. [{msg.role.value}] {msg.content}"
                    )
            else:
