...
class DebugApp(App):
    ...
    def on_mount(self):
        ...
        try:
            ...
            self.conversation.add_system_message(
                f"Model: {self.runner.config.model_provider}/{self.runner.config.model}"
            )
    ...
    async def test_backend_integration(self):
        ...
        try:
            ...
            self.conversation.add_system_message(
                f"[DEBUG] Backend returned {len(new_messages)} new messages"
            )

            if new_messages:
                for i, msg in enumerate(new_messages, 1):
                    self.conversation.add_system_message(
                        f"[DEBUG] Message {i}: [{msg.role.value}] {msg.content[:50]}..."
                    )
    ...
    def test_direct_message(self):
        ...
        self.conversation.add_assistant_message(
            "This is a test response from the UI itself (no backend)."
        )
