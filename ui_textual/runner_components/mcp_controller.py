    ...
    def _launch_autoconnect(self) -> None:
        ...
        def handle_completion(result: Optional[dict[str, bool]]) -> None:
            def finalize() -> None:
                self._connect_inflight = False
                if result is None:
                    self._enqueue_text("[yellow]Warning: MCP auto-connect failed.[/yellow]")
                    return
                if not result:
                    self._enqueue_text(
                ...
                if result:
                    ...
                    if successes:
                        lines.append(
                            "[green]✓ Connected MCP servers:[/green] " + ", ".join(successes)
                        )
                    if failures:
                        lines.append("[red]✗ Failed MCP servers:[/red] " + ", ".join(failures))
                    if lines:
                        self._enqueue_text("\n".join(lines))
