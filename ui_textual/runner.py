...
class TextualRunner:
    ...
    def _web_broadcast_sync(self, payload: dict) -> None:
        ...
        try:
            ...
            future = asyncio.run_coroutine_threadsafe(ws_mgr.broadcast(payload), loop)
    ...
    def _run_query(self, message: str) -> list[ChatMessage]:
        ...
        get_debug_logger().log("message_submitted", "runner", text=message[:200], source="user")
        ...
        try:
            ...
            if conversation_widget is not None:
                ...
                if getattr(self, "_web_server_thread", None) and self._web_server_thread.is_alive():
                    try:
                        ...
                        if web_loop and web_ws_manager:
                            ...
                            logger.warning("Bridge active but event loop/ws_manager unavailable")
                ...
                def _on_consumed(text):
                    ...
                    if self.app is not None and hasattr(self.app, "conversation"):
                        ...
                        if ledger:
                            ...
                            self.app.call_from_thread(self.app.conversation.add_user_message, text)
            ...
            _web_bridge_active = (
                getattr(self, "_web_server_thread", None) and self._web_server_thread.is_alive()
            )
        ...
        for msg in messages:
            if msg.role == Role.ASSISTANT:
                ...
                if content and not has_tool_calls:
                    ledger = getattr(self.app, "_display_ledger", None)
                    if ledger:
                        ledger.display_assistant_message(msg.content, "render_responses")
                    else:
                        logger.warning(
                            "DisplayLedger not available, falling back to direct "
                            "display (source=%s)",
