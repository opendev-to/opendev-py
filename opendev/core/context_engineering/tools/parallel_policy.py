...
READ_ONLY_TOOLS: frozenset[str] = frozenset({
    # File reading
    "read_file", "list_files", "search", "find_symbol", "find_referencing_symbols",
    "read_pdf", "analyze_image",
    # Process inspection
    "list_processes", "get_process_output",
    # Web (read-only)
    "fetch_url", "web_search", "capture_web_screenshot", "capture_screenshot",
    # Session/memory (read-only)
    "list_sessions", "get_session_history", "list_subagents", "memory_search",
    # Git (read-only actions)
    "git",  # Note: partitioner checks action param for git
    # Meta (read-only)
    "list_todos", "search_tools", "task_complete",
    # Agents listing
    "list_agents",
})
...
WRITE_TOOLS: frozenset[str] = frozenset({
    ...
    "memory_write",
    "kill_process",
    "write_todos", "update_todo", "complete_todo", "clear_todos",
