...
class AuthProfile:
    ...
    def mark_failure(self, status_code: int) -> None:
        ...
        logger.warning(
            ...
            self.key[:8], self.key[-4:], status_code, cooldown,
        )
...
class AuthProfileManager:
    ...
    def __init__(self, provider: str, keys: list[str]) -> None:
        ...
        self._profiles = [
            AuthProfile(key=k, provider=provider) for k in keys if k
        ]
    ...
    def get_active_key(self) -> Optional[str]:
        ...
        for i in range(len(self._profiles)):
            ...
            if profile.is_available:
                ...
                logger.info(
                    ...
                    profile.key[:8], profile.key[-4:], self._provider,
                )
                return profile.key
        ...
        logger.warning(
            ...
            len(self._profiles), self._provider, max(0, wait_time),
        )
        return None
