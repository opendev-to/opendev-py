...
class DefaultToolRenderer(
    ...
    def adjust_indices(
        self, delta: int, first_affected: int, skip_collapsibles: bool = False
    ) -> None:
        ...
        Args:
            ...
            skip_collapsibles: If True, skip adjusting collapsible outputs
                (used when collapsible.py already adjusted them internally)
        """
        ...
        if self._completed_single_agent is not None:
            if self._completed_single_agent.header_line >= first_affected:
                self._completed_single_agent.header_line += delta
            if self._completed_single_agent.tool_line >= first_affected:
                self._completed_single_agent.tool_line += delta
            for tl in self._completed_single_agent.active_tool_lines.values():
                if tl.line_number >= first_affected:
                    tl.line_number += delta
            if (
                self._completed_single_agent.overflow_line is not None
                and self._completed_single_agent.overflow_line >= first_affected
            ):
                self._completed_single_agent.overflow_line += delta
    ...
    def on_single_agent_start(self, agent_type: str, description: str, tool_call_id: str) -> None:
        ...
        """
        self._completed_single_agent = None
