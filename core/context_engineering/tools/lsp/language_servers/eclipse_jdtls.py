...
from opendev.core.context_engineering.tools.lsp.ls import LSPFileBuffer, SolidLanguageServer
...
class EclipseJDTLS(SolidLanguageServer):
    ...
    def __init__(
        ...
        shared_cache_location = str(
            PurePath(solidlsp_settings.ls_resources_dir, "lsp", "EclipseJDTLS", "sharedIndex")
        )
    ...
    def _setupRuntimeDependencies(
        ...
        vscode_java_path = str(
            PurePath(
                cls.ls_resources_dir(solidlsp_settings), dependency["relative_extraction_path"]
            )
        )
