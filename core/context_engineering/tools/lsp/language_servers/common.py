...
class RuntimeDependencyCollection:
    ...
    def __init__(
        self, dependencies: Sequence[RuntimeDependency], overrides: Iterable[Mapping[str, Any]] = ()
    ) -> None:
    ...
    def _run_command(command: str | list[str], cwd: str) -> None:
        ...
        if completed_process.returncode != 0:
            log.warning(
                "Command '%s' failed with return code %d", command, completed_process.returncode
            )
