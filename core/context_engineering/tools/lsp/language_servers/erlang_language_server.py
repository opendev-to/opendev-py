...
class ErlangLanguageServer(SolidLanguageServer):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            ProcessLaunchInfo(
                cmd=[self.erlang_ls_path, "--transport", "stdio"], cwd=repository_root_path
            ),
            "erlang",
            solidlsp_settings,
        )
    ...
    def _check_erlang_installation(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["erl", "-version"], check=False, capture_output=True, text=True, timeout=10
            )
    ...
    def _check_rebar3_available(cls) -> bool:
        ...
        try:
            result = subprocess.run(
                ["rebar3", "version"], check=False, capture_output=True, text=True, timeout=10
            )
            return result.returncode == 0
        except (subprocess.SubprocessError, FileNotFoundError):
            return False
