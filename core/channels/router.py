...
class MessageRouter:
    ...
    async def _handle_workspace_selection(
        ...
        if pending_key in self._pending_workspace_selection:
            pending_session, original_msg, available_workspaces = self._pending_workspace_selection[
                pending_key
            ]
            ...
            logger.info(f"Prompting workspace selection for {message.channel}:{message.user_id}")
    ...
    def _resolve_session(self, message: InboundMessage) -> Session:
        ...
        logger.info(f"Created new session {session.id} for {message.channel}:{message.user_id}")
        return session
    ...
    async def _dispatch_to_agent(self, session: Session, message_text: str) -> str:
        ...
        if asyncio.iscoroutinefunction(self._agent_executor):
            ...
            response = await loop.run_in_executor(None, self._agent_executor, session, message_text)
