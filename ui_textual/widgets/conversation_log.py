...
class ConversationLog(RichLog):
    ...
    def _coordinated_rerender(self) -> None:
        ...
        try:
            ...
            if cumulative_delta != 0:
                ...
                if (
                    self._plan_approval_start is not None
                    and self._plan_approval_start >= first_affected
                ):
    ...
    def toggle_output_expansion(self) -> bool:
        ...
        """
        toggled, delta, first_affected = self._tool_renderer.toggle_most_recent_collapsible()

        if delta != 0:
            self._spinner_manager.adjust_indices(delta, first_affected)
            self._tool_renderer.adjust_indices(delta, first_affected, skip_collapsibles=True)
    ...
    def get_single_agent_line(self) -> Optional[int]:
        """Get line of expandable single agent, or None."""
        if not self._tool_renderer.has_expandable_single_agent():
            return None
        agent = self._tool_renderer._completed_single_agent
        return agent.tool_line if agent else None

    def get_most_recent_collapsible_line(self) -> Optional[int]:
        """Get start line of most recent collapsible output, or None."""
        if not self._tool_renderer.has_collapsible_output():
            return None
        return self._tool_renderer._most_recent_collapsible
