...
class TestMainAgentTaskComplete:
                        ...
                        "tool_calls": [
                            {
                                ...
                                "function": {
                                    "name": "task_complete",
                                    "arguments": json.dumps(
                                        {"summary": "Failed to complete", "status": "failed"}
                                    ),
                                },
                            }
                        ],
    ...
    def test_loop_accepts_implicit_completion_on_success(self):
        ...
        responses = [
            ...
            # Second call - no tool calls (triggers implicit completion nudge)
            MagicMock(
                success=True,
                response=MagicMock(
                    status_code=200,
            ...
            # Third call - after nudge, accepts completion
            MagicMock(
                success=True,
                response=MagicMock(
                    status_code=200,
        ...
        with (
            patch.object(agent, "_priv_http_client") as mock_client,
        ...
        # Should nudge once with implicit_completion_nudge, then accept
    ...
    def test_loop_nudges_on_tool_failure(self):
        ...
        responses = [
            ...
            MagicMock(
                ...
                response=MagicMock(
                    ...
                    json=MagicMock(
                        return_value={
                            "choices": [
                                {"message": {"content": "I see the error...", "tool_calls": None}}
                            ],
                            "usage": {"total_tokens": 50},
                        }
                    ),
            ...
            # Fourth call - no tool calls (triggers implicit completion nudge)
            MagicMock(
                success=True,
                response=MagicMock(
                    status_code=200,
            ...
            # Fifth call - after nudge, accepts completion
            MagicMock(
                success=True,
                response=MagicMock(
                    status_code=200,
                    json=MagicMock(
                        return_value={
                            "choices": [
                                {"message": {"content": "Task complete!", "tool_calls": None}}
                            ],
                            "usage": {"total_tokens": 50},
                        }
                    ),
        ...
        # Should have nudged after failure, then nudged for implicit completion, then completed
