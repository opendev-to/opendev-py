...
class MockConversation:
    ...
    def on_parallel_agents_start(self, agent_infos):
        ...
        for info in agent_infos:
            print(
                f"  - {info['agent_type']}: {info['description'][:50]} (id={info['tool_call_id'][:16]}...)"
            )
...
def test_parallel_agent_tracking():
    ...
    second_agent_calls = [
        c for c in ui_callback._nested_calls if c["parent"] == "call_xyz789ghi012"
    ]
    assert (
        len(first_agent_calls) == 2
    ), f"Expected 2 calls for first agent, got {len(first_agent_calls)}"
    assert (
        len(second_agent_calls) == 1
    ), f"Expected 1 call for second agent, got {len(second_agent_calls)}"
    print(
        f"   ✓ Parent context matching works ({len(first_agent_calls)} + {len(second_agent_calls)} = {len(ui_callback._nested_calls)} tools)\n"
    )
...
def test_tool_filtering():
    ...
    assert filtered_tool_names == set(
        code_explorer_tools
    ), f"Expected {set(code_explorer_tools)}, got {filtered_tool_names}"
